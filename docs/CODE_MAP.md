# GamingCommander — Code Map

Quick reference for navigating the codebase. Avoids re-reading all source files after context compact.
...
└── tests/
    ...
    └── GamingCommander.App.Tests/   (added during 1.1a stabilization)
```
...
| `MigrationMode` | enum | `MoveOnly=0, MoveAndLink=1, ManifestRepairOnly=2` |
...
| `MigrationPlanSummary` | record | GameId, SourcePath, TargetPath, Mode, RequiresManifestBackup, RequiresLinkCreation, IsDryRunOnly |
...
- `ExeNameMatchesFolderName()` — substring + token match, beats size-based sort
...
| Tool | Purpose | Status |
|------|---------|--------|
| `parse_steam_acf.py` | Parse Steam ACF files, extract identification + migration fields | ✅ Exists, validated |
...
| `setup_mock_data.py` | Generate mock Windows game folder tree | 🆕 Added |
| `generate_mock_registry.py` | Generate mock .reg files for 5 launchers | 🆕 Added |
| `parse_registry.py` | Parse .reg files, extract launcher paths | 🆕 Added |
...
| `EGS_ITEM_FORMAT.md` | Epic .item JSON format + GraphQL API | ✅ Done |
| `epic_manifest_schema.md` | (not created yet — covered by EGS_ITEM_FORMAT.md) | ⏭️ Skipped |
| `pcgamingwiki_notes.md` | (not created yet) | ❌ Missing |
| `launcher_discovery.md` | (not created yet) | ❌ Missing |
