...
- **C# ALIGNMENT COMPLETE (2026-09-07):** all Python parity fixes ported to C# — terminating rule (T1.5, `FindExactFolderMatch` + Pass 1.5), whole-name match + backup guard (GAP A), parent-bound promotion (E4/E5, `FindParentBoundExe` + Pass 1.6), UE-container recognition (GAP B verified via HasUnrealLayoutSignal), BattleNet `.patch.result` (E2), `epicgames` noise refinement (E3, `IsNoiseExeName`), non-game layers 2-3 (`ContainerScanner.IsNonGameFolder`). New `PythonParityDetectionTests.cs` (10 tests). **C# 549 tests pass, 0 errors; Python matrix 22/22; baseline 13 clean.** Remaining C# gaps (LOW): 3-level subdir fallback, 4-level deep walk, DetectionLogger.
